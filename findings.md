...
- Extract `generateAutoNote` từ `components/trip-form.tsx` thành module độc lập `lib/quick-create/auto-note.ts` với options object signature. Mirror UI auto-note (`✨ Tạo thêm ghi chú`, `Xoá tất cả ghi chú`, preview realtime) từ trang add sang bottom sheet sửa bản nháp ở `/dashboard/quick-create`. 8 test cases mới (bao, 2C, vị trí đón/trả, 2k, format HHhMM/0-Xp, price < 1000, price là number) đều pass.
